...
function displaySocialShare($args = array())
{
    ...
    $social_links = '<div class="maat-social social-share' . $class . '"><div class="social-inner">';
    ...
    // Share on Facebook
    $social_links .= '<div class="social-icon"><a href="http://www.facebook.com/share.php?u=' . get_permalink() . '" title="Share this post on Facebook"  rel="nofollow" target="_blank"><i class="maat maat-facebook' . $icon_type . '"></i></a></div>';

    // Share on Twitter
    $social_links .= '<div class="social-icon"><a href="http://twitter.com/home?status=' . get_permalink() . '" title="Share this post on Twitter" rel="nofollow" target="_blank"><i class="maat maat-twitter' . $icon_type . '"></i></a></div>';

    // Share on Google
    $social_links .= '<div class="social-icon"><a href="https://plus.google.com/share?url=' . get_permalink() . '" title="Share this post on Google+" rel="nofollow"target="_blank"><i class="maat maat-google-plus' . $icon_type . '"></i></a></div>';

    // Share on LinkedIn
    $social_links .= '<div class="social-icon"><a href="http://linkedin.com/shareArticle?mini=true&url=' . get_permalink() . '" title="Share this post on LinkedIn" rel="nofollow" target="_blank"><i class="maat maat-linkedin' . $icon_type . '"></i></a></div>';

    // Share on Tumblr
    $social_links .= '<div class="social-icon"><a href="http://www.tumblr.com/share/link?url=' . get_permalink() . '" title="Share this post on Tumblr" rel="nofollow" target="_blank"><i class="maat maat-tumblr' . $icon_type . '"></i></a></div>';

    // Share on Pinterest
    $social_links .= '<div class="social-icon"><a href="http://pinterest.com/pin/create/button/?url=' . get_permalink() . '" title="Share this post on Pinterest" rel="nofollow" target="_blank"><i class="maat maat-pinterest' . $icon_type . '"></i></a></div>';
...
function displaySocialProfile($args = array())
{
    ...
    foreach ($profile_names as $profile) {
        if (!empty($myoptions[$profile])) {
            ...
            $icon_class = 'maat maat-' . $profile . $icon_type;
            $title = $site_name . ' ' . ucfirst($profile);
            $social_links .= '<div class="social-icon ' . $profile . '"><a href="' . $link . '" title="' . $title . '" target="_blank"><i class="maat maat-' . $icon_class . '"></i></a></div>';
        }
    }
