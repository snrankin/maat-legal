 ...
 * Last Modified: 4-17-19 at 5:23 pm
...
============================================================================ */
    ...
    const mainSass        = config.sass.main,
          ...
          modernizrConfig = config.modernizr;
...
============================================================================ */
    ...
    const jsValidate   = lazypipe()
                            .pipe(jshint, pkg.jshintFile).pipe(jshint.reporter, 'jshint-stylish'),
          minifyJS     = lazypipe()
                            .pipe(filter, mapFilter)
                            .pipe(rename, {
                                suffix: '.min'
    ...
    function compileJS(files, fileDest, fileBase = '') {
        ...
        .pipe(inject.prepend('var exports = {};\n'))
        .pipe(concat('temp.js'))
    ...
    function compileModernizr(done) {
        var validate  = true,
            errorFree = true,
            hasErrors = false;
        return src([
        ...
        .pipe(modernizr('01-modernizr.js', modernizrConfig))
        .pipe(beautify.js({
            indent_size: 4
        }))
        .pipe(gif(isProd, minifyJS()))
    ...
    function createVendorJSList() {
        var vendorJSarray = [];
        Object.values(vendorJS).forEach((item, index) => {
            vendorJSarray[index] = item;
        });
        var vendorJSarray = vendorJSarray.flat();
        //var vendorJSarray = JSON.stringify(vendorJSarray);
    ...
    function createVendorJSArray() {
        var vendorJSarray = [];
        Object.entries(vendorJS).forEach( (entry, i) => {
            let index = i + 2;
            let key = entry[0];
            let value = entry[1];
    ...
    exports.pageScripts       = compilePageJS;
    exports.scripts           = parallel(mainScripts, adminScripts, series(compileModernizr, copyVendorJS, compileVendorJS), compilePageJS);
...
============================================================================ */
    ...
    exports.mainBuild   = parallel(mainStyles, mainScripts, series(compileModernizr, copyVendorJS, compileVendorJS), images);
    exports.pageBuild   = parallel(pageStyles, compilePageJS);
    exports.build       = parallel(images, series(compileModernizr, copyVendorJS, compileVendorJS), adminStyles, adminScripts, mainStyles, mainScripts, pageStyles, compilePageJS);